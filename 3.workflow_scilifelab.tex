...
There is also a dedicated download server which has access to the NFS export. This download server is accessible from  the Internet and allow downloads via rsync (over SSH) or SFTP. 
...
The described session wizard can be found at  \url{https://github.com/delarosatrevin/scipion-session}. It has been designed to satisfy the requirements for \scilifelab and, although readers are welcomed to try it, no support is offered.  \emph{Booked Scheduler} is available at (\url{https://www.bookedscheduler.com/})
